<?php
    ...
    $evalOpenData = explode('***', $decodedData);//[0][1]

    $typeOfEvent = $evalOpenData[0];
    $openEventID = $evalOpenData[1];
    //echo $openEventID;
    $openEventName = $evalOpenData[2];
    
    $getStatusEval = mysqli_query($connection, "SELECT evaluation_id, status FROM evaluation_list WHERE event_id = '$openEventID'");
    if(mysqli_num_rows($getStatusEval) > 0){
        $statusEval = mysqli_fetch_array($getStatusEval);

        $evaluationID = $statusEval[0];
        $statusEvaluation = $statusEval[1];

        //para overwrite nalang sa destinationqr
        $checkOpenEventQR = mysqli_query($connection, "SELECT openEventQR FROM event_openqr WHERE event_id = '$openEventID'"); 
        if(mysqli_num_rows($checkOpenEventQR) > 0){
            $openEventQR = mysqli_fetch_array($checkOpenEventQR);

            if(empty($openEventQR[0])){//new generated qr
                $qrName = uniqid('',true).'.png';
            }
            else{
                $qrName = $openEventQR[0];
            }
            ...
            $updateOpenEventKey = "UPDATE event_openqr SET openEventQR = '$qrName', keyScan='$key', dateTime_generated='$currentDateTime' WHERE event_id = '$openEventID'";
            if(mysqli_query($connection, $updateOpenEventKey)){

                $openEventQRDirectory = "../openEventQR/";

                $qrEncdata = $typeOfEvent . "***" . $openEventID . "***" . $openEventName . "***" . $evaluationID . "***" . $statusEvaluation . "***" . $key;
                //echo $qrEncdata;
                ...
                file_put_contents($openEventQRDirectory.$qrName, $rawImage);
        ...
        echo 'ayaw no evaluation openqr';
    }
            ...
            <div class="container-fluid">
                ...
                <div class="printable shadow-lg " id="pdfGenerate">
                    ...
                    <div id="content" class="px-3">
                        ...
                        <div class="row">
                            <h1 class="text-center"><?php echo $openEventName;?></h1>
                        </div>
                        <div class="row">
                            <h1 class="text-center"> Event QR Code</h1>
                        </div>
                        
                        <div class="d-flex justify-content-center">
                            <img class="qrPrint" src="../openEventQR/<?php echo $qrName;?>" alt='QR Code'>
                        </div>
                    </div>
                    ...
                    <div class="row border border-dark border-start-0 border-end-0 border-bottom-0" id="footerPrintEvent">
                        <div class="col text-center" style="background-color: white">
                            <span>THIS QR CODE IS INTENDED EXCLUSIVELY FOR INDIVIDUALS WHO ATTENDED THE "<i><?php echo $openEventName;?></i> "EVENT</span>
                        </div>
                    </div>
    ...
    <script>
        ...
        function saveAsPDF() {
            ...
            html2canvas(content, { scale: 2}).then(function(canvas) {
                ...
                doc.save('<?php echo $openEventName. " (" . $dateToday . ")"?>.pdf');
            });
