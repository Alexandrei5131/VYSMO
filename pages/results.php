<?php
    ...
    $getAllQuestion = mysqli_query($connection, "SELECT * FROM evaluation_question WHERE evaluation_id = '$evaluationID'");
    if(mysqli_num_rows($getAllQuestion) > 0){
        $numOfQuestion = mysqli_num_rows($getAllQuestion);

        while($row = mysqli_fetch_array($getAllQuestion)){
            ...
            <div class="container-fluid bodyContent mt-3 col-11"  >
                    <div class="bg-light text-center rounded">
                        <nav class="navbar navbar-light justify-content-center visitInfo">
                            <span class="navbar-brand mb-0 text-white"><?php echo $eventName. ' ';?>Evaluation Results</span>
                        </nav>
                        <div class="p-4">
                            ...
                            <table class="table text-start align-middle table-striped table-hover mb-0 shadow p-3 rounded">
                                <thead class="sticky-top text-center shadow p-3 mb-5 rounded" style="z-index: 2;">
                                    <tr class="text-dark">
                                        ...
                                        <th scope="col">Verbal Interpretation</th>
                                ...
                                <tbody  id="tData" class="text-center">
                                    ...
                                    <?php
                                        ...
                                        for($i=0, $num=1; $i < $numOfQuestion; $i++, $num++){
                                            ?>
                                            
                                            
                                            <?php
                                            $sqlGetScaleAndNumOfEvaluate = mysqli_query($connection, "SELECT AVG(scaleID) FROM evaluate_answer WHERE evaluation_id = '$evaluationID' AND questionID = '".$question[$i]['questionID']."'");
                                            $scaleScore = mysqli_fetch_array($sqlGetScaleAndNumOfEvaluate);
                                            if ($scaleScore) {
                                                $averageScaleScore = $scaleScore[0];
                                            } else {
                                                $averageScaleScore = "N/A"; // or some default value if no result is found
                                            }
                                    ?>
                                            <tr>
                                                <td><?php echo $num;?></td>
                                                <td><?php echo $question[$i]['question'];?></td>
                                                <td><?php echo $averageScaleScore; ?></td><!--computed score-->
                                                <td>
                                                    <?php
                                                    
                                                        if($averageScaleScore >= 1.00 && 1.80 >= $averageScaleScore){
                                                            echo "Very Dissatisfied";
                                                        }
                                                        elseif($averageScaleScore >= 1.81 && 2.60 >= $averageScaleScore){
                                                            echo "Dissatisfied";
                                                        }
                                                        elseif($averageScaleScore >= 2.61 && 3.40 >= $averageScaleScore){
                                                            echo "Neutral";
                                                        }
                                                        elseif($averageScaleScore >= 3.41 && 4.20 >= $averageScaleScore){
                                                            echo "Satisfied";
                                                        }
                                                        elseif($averageScaleScore >= 4.21 && 5.00 >= $averageScaleScore){
                                                            echo "Very Satisfied";
                                                        }

                                                    ?>    
                                                </td><!--verbal interpretation depending in score-->
                                                <td>
                                                    <form action="responses.php?">
<!--tago-->                                             <input type="hidden" name="evaluationID" value="<?php echo $evaluationID;?>">
                                                        <input type="hidden" name="eventName" value="<?php echo $eventName;?>">
                                                        <input type="hidden" name="questionID" value="<?php echo $question[$i]['questionID'];?>">
                                                        <button type="submit" class="btn btn-success" name="">View Responses</button>
                                                    </form>
                                                </td>
